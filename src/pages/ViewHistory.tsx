...
export default function ViewHistory() {
  ...
  const [reportStatus, setReportStatus] = useState<
    "all" | "present" | "leave" | "kitchen_duty" | "absent"
  >("all");
  ...
  useEffect(() => {
    ...
    const fetchRecords = async () => {
      ...
      for (
        let d = new Date(accountCreatedAt);
        d <= today;
  ...
  const fetchAttendanceData = async () => {
    ...
    const kitchenDuty =
      data?.filter((r) => r.status === "kitchen_duty").length || 0;
    ...
    const elapsedDays = Math.max(
      1,
      Math.floor((now.getTime() - accountCreatedAt.getTime()) / 86400000) + 1
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row mb-[100px] pt-20">
      ...
      <div className="container mx-auto p-6 md:ml-64 text-sm md:text-base lg:text-lg">
        <h1 className="font-bold mb-6 justify-center items-center flex text-xl md:text-2xl lg:text-4xl">
        ...
        <div className="flex flex-col sm:flex-row flex-wrap justify-center items-center gap-4 mb-6">
          <select
            ...
            className="border-[2px] border-foreground px-4 py-3 rounded-md"
          >
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" className="px-4 py-2">
                From:{" "}
                {reportFromDate ? format(reportFromDate, "PPP") : "Pick date"}
              </Button>
            </PopoverTrigger>
            <PopoverContent>
              <Calendar
          ...
          <div className="overflow-x-auto mb-6">
            <table className="w-full border-collapse border-[2px] border-foreground">
              <thead className="bg-gray-100">
                <tr>
                  <th className="border-[2px] border-foreground px-3 py-3">
                    From
                  </th>
                  <th className="border-[2px] border-foreground px-3 py-3">
                    To
              ...
              <tbody>
                {currentRecords.map((rec, idx) => (
                  <tr key={idx} className="text-sm text-center">
                    <td className="border-[2px] border-foreground px-3 py-3">
                      {new Date(rec.from).toLocaleDateString()}
                    </td>
                    <td className="border-[2px] border-foreground px-3 py-3">
                      {new Date(rec.to).toLocaleDateString()}
                    </td>
                    <td className="border-[2px] border-foreground px-3 py-3 capitalize">
                      {rec.status}
                    </td>
                  </tr>
                ))}
